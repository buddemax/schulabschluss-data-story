...
- **20 Measures** (in `dim_abschluss.tmdl`, inkl. `Schüleranteil ohne Grundschule %` für die LF5-Fokussicht und der beiden LF4-Delta-Measures `Gap ohne HSA (pp)`/`Gap Abitur (pp)`), je gegen Referenzwert (`data/kpi_referenzwerte.json`) geprüft; darunter der 3-dimensionale Risiko-Score (LF9, inkl. Einkommen), StdAbw Quote ohne HSA (Kreise) (LF3) und Verf. Einkommen je EW Ø. Hinzu kommen **sechs reine Formatierungs-Measures** (Präfix `Farbe …`, nicht analytisch), die je Visual die Kernaussage akzentuieren (LF1 Sachsen-Anhalt, LF2 Anhalt-Bitterfeld, LF5 Grundschulen/Gymnasien, LF6 Rangwechsel, LF9 Top-10-Risiko) – Conditional Formatting „nach Feldwert".
...
- **Interaktivität:** **1 geografische Deutschlandkarte** auf Kreisebene (LF2, Bubble-Map, Blasengröße = Quote ohne HSA; Werkzeug-Grenze Choropleth s. `BEFUNDE_UND_KORREKTUREN.md`). **6 gezielt eingesetzte Slicer** (bewusst entschlackt): Schuljahr (LF1, steuert den Trend-Chart), Bundesland-Einzelauswahl (LF5/LF6/LF7/LF9), Einkommens-Schieberegler (LF9, Between-Modus). Drilldown über die Region-Hierarchie (Land→RB→Kreis). Die Visuals einer Seite cross-filtern sich; auf LF8 ist der Stadtstaaten-Confounder über die farbliche Punkt-Trennung direkt ablesbar. Der Bericht umfasst **10 Seiten** (Überblick + LF1–LF9).
...
- 20 analytische DAX-Measures angelegt und gegen die Referenzwerte geprüft; Datenfehler DQ8/DQ10/DQ11 behoben (s. `dq_report.md`).
